...
public class CheckInPlugin implements Plugin {
	...
	@Autowired
	public CheckInPlugin(AdvancedUserDao<String> userDao, ActionCrud actionCrud, AdvancedElementDao<String> elementDao , UpdateProperties up) {
		super();
	...
	public Object doSomething(ActionEntity action) throws Exception {
		...
		List<ActionEntity> checkOutList = this.actionCrud
				.findAllByActionTypeAndElementSmartspaceAndElementIdAndPlayerSmartspaceAndPlayerEmail("CheckOut",
						action.getElementSmartspace(), action.getElementId(), action.getPlayerSmartspace(),
						action.getPlayerEmail());

		if (checkInList.isEmpty() || checkInList.size() == checkOutList.size()) {
			...
			return new StringActionResponse("Checked In !");
		} else
